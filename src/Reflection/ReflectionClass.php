...
class ReflectionClass implements Reflection
{
    ...
    protected function __construct(Reflector $reflector, $node, LocatedSource $locatedSource, ?string $namespace = null)
    {
        ...
        $parentClassName       = $node instanceof ClassNode ? ($nullsafeVariable1 = $node->extends) ? $nullsafeVariable1->toString() : null : null;
    ...
    public static function importFromCache(Reflector $reflector, array $data): self
    {
        ...
        $ref->immediateProperties = array_map(
            static fn ($propData) => ReflectionProperty::importFromCache($reflector, $propData),
            $data['immediateProperties'],
        );
        $ref->immediateMethods = array_map(
            static fn ($methodData) => ReflectionMethod::importFromCache($reflector, $methodData),
    ...
    private function createMethodsFromTrait(ReflectionMethod $method, array $currentMethods): array
    {
        ...
        $createMethod = function (?string $aliasMethodName, int $methodModifiers) use ($method): ReflectionMethod {
    ...
    private function getPropertiesConsideringAlreadyVisitedClasses(AlreadyVisitedClasses $alreadyVisitedClasses): array
    {
        ...
        $properties = array_merge(
            array_filter(
                (($nullsafeVariable2 = $this->getParentClass()) ? $nullsafeVariable2->getPropertiesConsideringAlreadyVisitedClasses($alreadyVisitedClasses) : null) ?? [],
